...
```{r message=FALSE}
vdjdb_mhc1 = fread("datasets/vdjdb.slim.txt") %>%
...
vdjdb_mhc2 = fread("datasets/vdjdb.slim.txt") %>%
  filter(species == "HomoSapiens", gene == "TRB", mhc.class == "MHCII") %>%
  mutate(cdr3aa = cdr3, hla = str_split_fixed(mhc.a, "[-:]", 3)[,2]) %>%
  select(cdr3aa, antigen.epitope, hla, antigen.species)
...
```{r message=FALSE}
annotated_1 = datasets %>%
  merge(vdjdb_mhc1)



n_nonnaive = annotated_1 %>% group_by(sample_id) %>%
  filter(antigen.species != 'HomoSapiens') %>% summarise(fl = sum(freq)) %>% arrange(fl)
n_nonnaive$naive = rep(c(T, F), ea = 8)
n_nonnaive = n_nonnaive %>% select(-fl)

annotated_2 = datasets %>%
  merge(vdjdb_mhc2)

n_nonnaive = annotated_2 %>% group_by(sample_id) %>%
  filter(antigen.species != 'HomoSapiens') %>% summarise(fl = sum(freq)) %>% arrange(fl)
# n_nonnaive$naive = rep(c(T, F), ea = 8)
n_nonnaive = n_nonnaive %>% select(-fl)
...
```{r}
annotated_1[annotated_1$antigen.species == 'CMV',] %>% group_by(sample_id) %>% 
  summarise(fr = sum(freq)) %>% arrange(desc(fr))
```
...
# fig.height=8, fig.width=12}
  annotated_2 %>% 
  # filter(antigen.species != 'HomoSapiens', !(sample_id %in% c('s3', 's7')) ) %>%
